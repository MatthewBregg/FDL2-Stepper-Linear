...
// See http://torukmakto4.blogspot.com/2018/01/google-drive-link-for-hy-con-part-files.html for original the linear stepper source.

SYSTEM_MODE(AUTOMATIC);

// Constants from /u/torukmakto4's code
 //FEED DELAY PARAMETERS - increase feedDelayBase if you have low first shot velocity. Decrease for quicker lock time (HvZ players take note)
 //Increase recentShotCompensation for shorter lock time on followup shots within 1s - decrease for more consistent lock time.
 //DEFAULTS are feedDelayBase=140 and recentShotCompensation=60 for 9.5mm Gen3 wheels, on 3S
 int feedDelayBase = 140; //ms
 int recentShotCompensation = 60; //ms - delay reduction when recent torque command to flywheel motors
...
#define pusherStep D4 // Keep the above a compile time constant for speed purposes.
